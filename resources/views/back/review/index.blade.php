...
@php($count = (int) ($doctor->revisions_num ?? 0))
<tr><td><div class="hn-person"><span class="hn-avatar">{{ mb_substr($doctor->name_ar ?: 'ط', 0, 1) }}</span><div><strong>{{ $doctor->name_ar ?: $doctor->user->name }}</strong><small>{{ $doctor->specialization_ar ?: 'غير محدد' }}</small></div></div></td><td>{{ $doctor->gnr_m_clinics?->name_ar ?: 'غير محددة' }}</td><td>{{ number_format($count) }}</td><td>@if($count > 0)<span class="hn-badge hn-badge-pending"><i class="fe fe-star"></i> {{ number_format(((float) $doctor->total_rate) / $count, 1) }} / 5</span>@else — @endif</td></tr>
@endforeach
